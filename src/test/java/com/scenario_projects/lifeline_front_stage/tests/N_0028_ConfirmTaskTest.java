...
public class N_0028_ConfirmTaskTest extends BaseTest {
    private String timeData, taskName;
    private String confirmTask = "Done";
    ...
    public void confirmTask() {
        ...
        taskPage.clickConfirmTaskButton();

        Assert.assertTrue(taskPage.editTaskPopupIsPresent(), "Failed! Undone task popup is appeared!");
        ...
        Assert.assertTrue(taskPage.checkTaskStatus(confirmTask), "Failed! Status task doesn't equal " + confirmTask);

        patientDetailPage.clickBackButton();
        Assert.assertTrue(patientDetailPage.listDayTasksIsPresent(), "Failed! Undone task hasn't appear on the patient detail page!");
    }
